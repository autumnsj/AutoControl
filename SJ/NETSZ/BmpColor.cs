...
namespace NETSZ
{
    public class BmpColor
    {
        ...
        public static List<Point> FindPicScreen(int x1, int y1, int x2, int y2, string fileName, int similar)
        {   
            Rectangle rect=new Rectangle(x1,y1,x2-x1,y2-y1);
            Bitmap S_bmp = CopyScreen(rect);
            Bitmap P_bmp = new Bitmap(fileName);
            List<Point> temp= FindPic(0, 0,rect.Width,rect.Height, S_bmp, P_bmp, similar);
            List<Point> result = new List<Point>();
            foreach (var item in temp)
            {
                result.Add(new Point(item.X+x1,item.Y+y1));
            }
            return result;
        }
        public static void FindPicScreen(int x1, int y1, int x2, int y2, string fileName, int similar,out int x,out int y)
        {
            Rectangle rect = new Rectangle(x1, y1, x2 - x1, y2 - y1);
            Bitmap S_bmp = CopyScreen(rect);
            Bitmap P_bmp = new Bitmap(fileName);
...
/// <summary>
        ...
        /// <param name="rect"></param>
        /// <returns></returns>
        public static Bitmap CopyScreen(int x1, int y1, int x2, int y2)
        {
            Rectangle rect = new Rectangle(x1, y1, x2 - x1, y2 - y1);
